...
**Then:**
- OTEL SDK initializes successfully using `config.EnvEnv()` env vars (OTEL_SERVICE_NAME, OTEL_TRACES_EXPORTER, etc.)
- `otelhttp.NewServerHandler()` wraps all HTTP routes and automatically creates spans for every incoming request
- No manual span instrumentation appears in any business logic (`internal/store`, `internal/http/handler.go`) — this is purely middleware-level auto-instrumentation
- The server still serves `/api/columns/` correctly (no regression)

**Scenario 1.1: Unconfigured OTEL env vars**  
**Given:** No OTel env vars are set  
**When:** the application starts  
**Then:** it initializes with noop exporters (no panics, no errors in stderr) and continues serving HTTP normally
...
**Then:** the span's resource attributes include all of:
...
**Scenario 2.1: Partial env var coverage**  
**Given:** Only `OTEL_SERVICE_NAME=kanban-backend` is set (no DEPLOYMENT_ENVIRONMENT or POD_ID)  
**When:** a span is created  
**Then:** `service.name = "kanban-backend"` appears; missing attributes simply do not appear (no default values injected, no errors)
...
**Then:**
- The span attribute key is `"http.method"` (NOT `"http_method"`) per semconv 1.20+
- The span name follows the convention `"<METHOD> <path_template>"` (e.g., `"GET /api/columns/"`, NOT a unique URL path)
- No cardinality-exploding attributes appear automatically, such as full query strings or user IPs

**Scenario 3.1: POST with UUID in path**  
**Given:** A request `POST /api/columns/some-uuid-here` is sent  
**When:** the span is created  
**Then:** the span name is `"POST /api/columns/"` (path template, NOT the actual UUID)
...
**Then:**
- No errors or warnings appear from `semconv.validate()` on the exported resource attributes / span tags
- The validation script rejects configs that add high-cardinality attributes (e.g., `"http.full_url"`, custom user fields as span attrs)

**Scenario 4.1: Deliberately invalid config**  
**Given:** A collector config adds `http.route` as a non-semconv tag to trigger cardinality  
**When:** the validation check runs  
**Then:** it exits with a non-zero status and prints a rejection message about high-cardinality risk

---

## Implementation Notes

- **Where it lives:** `/backend/main.go` — setup OTEL SDK, configure `otelhttp.NewServerHandler()`, wire into standard `net/http` router
- Environment variables: `OTEL_TRACES_EXPORTER=otlp`, `OTEL_METRICS_EXPORTER=prometheus`, `OTEL_LOGS_EXPORTER=console`
