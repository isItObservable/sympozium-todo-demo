...
**Then:** that call's Authorization header MUST be:
**- Set via `otelhttp.NewClient()` which injects `traceparent` (W3C TraceContext) and `authorization` headers into the outgoing request**
...
**Given:** An upstream service sends a request with `traceparent` header  
...
**THEN**: the new span's trace_id matches that provided by the `traceparent`
**AND:** the parent_span_id is set correctly from the header
**AND:** if no existing `traceparent` exists, a NEW trace is started — this prevents errors / panic
...
**THEN**: a complete waterfall shows the incoming HTTP span as the parent node, spanning to the note handler as a child span which then calls store functions
**AND:** each service topology is correctly rendered with distinct service names (kanban-backend → kanban-note-svc, etc.)
**AND:** no orphaned / disconnected spans exist — all linked via trace relationships.
...
- `board.id` UUID of the target resource
