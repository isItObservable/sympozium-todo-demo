...
**Alerts to configure:**
- `http_server_duration_seconds:p99 > 1s` on any backend → frontend path (per OBE-2 AC2)
...
**AND**: saturation metrics from OTel process (Goroutine count if available via `process_` metrics).  
**AND**: A Loki/LogQL query showing logs correlated by `trace_id` — filtering a single trace ID in Grafana instantly shows all related backend + frontend log entries for that trace.
...
- `P0 — http.server.duration p99 > 1s on any backend → frontend HTTP path` (duration alert as per OBE-2 AC2 + P0 status)
 - **AND**: `http_server_request_count_total / total_requests_by_endpoint > 5% sustained over 5 minutes for 5xx error alerts.
**AND**: HTTP server active connections approaching collector capacity (active connection metrics threshold, e.g., approaching exporter's maximum concurrent connections or queue depth).
...
**THEN:** it returns all log entries from **both** backend AND frontend that share the same trace context
**AND:** no false positives appear (other traces should NOT match)

### AC4: Dashboards + alerts are reproducible as code (no manual config)
...
**THEN:** the exact same dashboards and alert rules appear in Grafana without clicking through any UI wizards  
**AND**: dashboard JSON files are committed to git as source-of-truth (not hand-made via the Grafana web UI)
